...
def updateWeights(X,y_pred,y_true,W,alpha,index):
  """
  Parameters:
  X (array) : Independent Features
  y_pred (array) : Predicted Target Variable
...
def train(X, y, epochs=10, alpha=0.001, random_state=0):
    ...
    for j in range(epochs):
      ...
      for i in train_indices:
        loss, y_pred = forward(X[i],y[i],W[0])
        cost+=loss
        W[0] = updateWeights(X,y_pred,y,W[0],alpha,i)
      train_loss.append(cost)
      num_epochs.append(j)
      return W[0], train_loss, num_epochs

def test(X_test, y_test, W_trained):
    """
    Parameters:
    ...
    for i in test_indices:
        loss, y_test_pred = forward(X_test[i], W_trained, y_test[i])
...
def linear_regression():
    #Get the dataset
  dataset = pd.read_csv(r'weatherHistorysmall.csv')
  ...
  W_trained, train_loss, num_epochs = train(X_train,y_train, epochs=1000, alpha=0.0001)
  #Testing on the Test Dataset
  test_pred, test_loss = test(X_test, y_test, W_trained)
  # Visualizing Results
  # Plot the Train Loss
